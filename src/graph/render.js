...
export default class Render {
    ...
    cancelRunningAnimation(handleCall) {
        if (this._runningAnimation && this._runningAnimation.isRunning) {
    ...
    // 不再迭代绘制子节点,而是绘制队列
    drawQueue(ctx) {
        this.renderQueue.forEach(child => {
            child.draw(ctx);
        })
    }
